...
class SequenceModelEvaluator(AbstractModelEvaluator, ABC):
    ...
    def eval_model(self):
        for train, val, test in self.k_fold():
            ...
            compute_probabilities(self._model, train, self.get_model_train_folder())
            compute_probabilities(self._model, val, self.get_model_val_folder())
            compute_probabilities(self._model, test, self.get_model_test_folder())
    ...
    def k_fold(self):
        ...
        for train, val_test in k_fold.split(labels):
            ...
            tf.print(f'{self}: The train index is {train}')
            tf.print(f'{self}: The val index is {val}')
            tf.print(f'{self}: The test index is {test}')
