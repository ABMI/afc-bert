...
@log_function_decorator
def compute_probabilities(model, test_data: Union[Dataset, Tuple[np.ndarray, np.ndarray]],
                           proba_folder):
    """
    Compute Recall, Precision, F1-score and PR-AUC for the test data
    ...
    def data_prep():
        if isinstance(test_data, Dataset):
            x = test_data.map(lambda _x, _y: _x)
            x = list(chain(*x.as_numpy_iterator()))
        elif len(test_data) == 2:
            x, y = test_data
        else:
            raise TypeError('Only numpy array and tensorflow Dataset are supported types.')
    ...
    current_time = datetime.datetime.now().strftime("%m-%d-%Y-%H-%M-%S")    
    validate_folder(proba_folder)
    
    probabilities, labels = run_model()
    x = data_prep()
    
    create_folder_if_not_exist(proba_folder, 'probabilities')
    #pd.DataFrame(test_data.map(lambda _x, _y: _x)).to_parquet(os.path.join(proba_folder, 'probabilities', 'data_all_'+f'{current_time}.parquet'))
    #np.savetxt(os.path.join(proba_folder, 'probabilities', 'data_x_'+f'{current_time}.txt'), x, fmt='%d')
    #pa_table = pa.table({"data": x})
    #pq.write_table(pa_table, os.path.join(metrics_folder, 'probabilities', 'data_x_'+f'{current_time}.parquet'))
    #pd.DataFrame(x, columns = ['x']).to_parquet(os.path.join(proba_folder, 'probabilities', 'data_x_'+f'{current_time}.parquet'))
    pd.DataFrame(labels, columns = ['y']).to_parquet(os.path.join(proba_folder, 'probabilities', 'data_y_'+f'{current_time}.parquet'), index = True)
    pd.DataFrame(probabilities, columns = ['Probabilities']).to_parquet(os.path.join(proba_folder, 'probabilities', f'{current_time}.parquet'), index = True)
